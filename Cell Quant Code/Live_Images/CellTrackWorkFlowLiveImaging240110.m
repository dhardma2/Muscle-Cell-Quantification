...
%David Hardman, University of Edinburgh 2021

clear;
nf=0;
%input video number for tracking and image parameters
prompt = {'Pixel width (μm):','Frame time length (mins):','Number of first video:',...
    'Number of last video:','Filename suffix:','File type (eg .tif)','Position:','Sample field width (μm):',...
    'Max cell search radius (μm):','Myotube size threshold (μm):', 'Minimum myoblast area (pixels):'};
dlgtitle = 'Input';
dims = [1 35];
definput = {'0.454','5','1','1','_Myotube fusion Day0-1 Diff+0HS_','.tif','2','1000','45','80','300'};
...
filetype=char(answer(6));
position=str2double(answer(7));
FoV=str2double(answer(8));
RoI=str2double(answer(9));
crit_myoc_len_um=str2double(answer(10));
Min_myoblast_area=str2double(answer(11));
...
for v=1:(vid2-vid1+1)
    ...
    vidno=vid1+(v-1);
filename= sprintf('stk_%s%d%spos%d%s',pref,vidno,suffix,position,filetype);
info=imfinfo(filename);
numframe=length(info);
%image size
flength=round(FoV/ps,0);
fwidth=round(FoV/ps,0);
for f=1:numframe
    nf=nf+1;
    g=(v-1)*numframe+f;
    %ff=f+90;
...
[Myoblasts,Myotubes,NumMyoblasts,NumMyotubes,statsMB2,statsMT2]=Masks(Iin,x1,x2,x3,Amax,f,ps,crit_myoc_len_um,Min_myoblast_area);
...
for f=2:nf
    f
Iin2=cooked(:,:,:,f);
[Myoblasts,Myotubes,NumMyoblasts,NumMyotubes,statsMB2]=Masks(Iin2,x1,x2,x3,Amax,f,ps,crit_myoc_len_um,Min_myoblast_area);
...
function [Myoblasts,Myotubes,NumMyoblasts,NumMyotubes,statsMB2,statsMT2]=Masks(Iin2,x1,x2,x3,Amax,f,ps,crit_myoc_len_um,Min_myoblast_area)
%critical length of myocyte before it is classed as a myotube
crit_myoc_len=crit_myoc_len_um/ps;
crit_myoc_len=round(crit_myoc_len,0);
Amin=round(Min_myoblast_area,0);
...
Myoblasts=bwareaopen(Myoblasts,round(Amin*1.17,0));
...
%remove areas less than threshold minimum pixels in area
BW2=bwareaopen(BW2,Amin);
%figure
%imshow(BW2)
BW4(:,:,1)=BW2;
...
[sizex]=size(Iin(:,1,:,1));
[sizey]=size(Iin(1,:,:,1));
for row = 1 : 500 : sizex(1)
 line([1, sizey(2)], [row, row]);
end
for column = 1 : 500 : sizey(2)%3
