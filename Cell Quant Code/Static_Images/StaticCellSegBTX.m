...
prompt = {'Pixel width (microns):','Day:','Start Position:','Enhancement factor:','image length:','Dilation factor:','% Pericentrin threshold:','File_Prefix:'};
dlgtitle = 'Input';
dims = [1 35];
%Pericentrin threshold will change with fluorescence intensity
definput = {'0.227','2','32','20','500','3','0.05','HiPSC'};
...
filename1=sprintf('%s%d-MTDNA.tif',pref,p);
filename2=sprintf('%s%d-DNABW.tif',pref,p);
filename3=sprintf('%s%d-DNA.tif',pref,p);
filename4=sprintf('%s%d-MTDNABW.tif',pref,p);
filename5=sprintf('%s%d-ActinBW.tif',pref,p);
filename6=sprintf('%s%d-BTX.tif',pref,p);
